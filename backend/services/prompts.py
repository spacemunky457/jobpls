...
def build_match_prompt(
    ...
    return (
        ...
        + "ROLE-FIT WEIGHTING — the tier must reflect fit with what the candidate WANTS, not only what "
        "they could technically do. Weigh their stated preferences, deal-breakers and target priorities "
        "as heavily as raw skill overlap. If a role's CORE focus matches one of the candidate's explicit "
        "deal-breakers or anti-preferences (e.g. they want systems/automation/ownership and the role is "
        "pure manual or traditional software-QA testing, pure software/backend engineering, data science, "
        "or pure account/customer management with no tooling ownership), it is a POOR fit: cap it at "
        "'stretch', or 'skip' if it squarely is what they said they want to avoid — even when the skill "
        "overlap is strong and they could do the work. Tier up only when the role centers on their target "
        "niches. When a role 'could work technically' but conflicts with their stated intent, choose the "
        "lower tier and name the conflict in the verdict.\n\n"
